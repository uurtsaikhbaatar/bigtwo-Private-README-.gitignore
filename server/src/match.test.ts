...
test('өнжих ээлжтэй үед 3♦-тэй тоглогч эхэлнэ, гэхдээ 3♦-ээ тавих албагүй', () => {
  ...
  for (let round = 0; round < 4; round++) {
    const starter = state.players.find((p) => p.id === state.seats[state.turn])!;
    assert.ok(starter.hand.includes(0), `${round + 1}-р дугуй: 3♦-тэй хүн эхэлнэ`);
    assert.equal(state.openWithThree, false, '4-өөс олон тоглогчтой үед 3♦ албадахгүй');
...
test('өнжих ээлж дуусахад 3♦ сүүлийн нэг удаа эхлэгчийг заана', () => {
  ...
  // Ээлж дууссаны дараах ЭХНИЙ дугуй: 3♦ заана, хожигч биш.
  startRound(state, rng);
  ...
  assert.equal(state.openWithThree, false, 'энд ч 3♦-ээ албадахгүй');

  // Дараагийн дугуйгаас эхлэн хожигч гарна.
  playOutRound(state, rng);
  const winner = state.lastRoundWinnerId!;
  startRound(state, rng);
  assert.equal(state.seats[state.turn], winner, 'үүнээс хойш хожигч эхэлнэ');
  assert.equal(state.openWithThree, false);
});
...
test('3♦ тарааагдаагүй бол хамгийн бага хөзөртэй тоглогч эхэлнэ', () => {
  ...
  for (let seed = 1; seed <= 40 && !found; seed++) {
    ...
    startMatch(state, 200, mulberry32(seed));
    ...
    assert.equal(state.openWithThree, false, '3♦ байхгүй тул албадах зүйлгүй');
  }
