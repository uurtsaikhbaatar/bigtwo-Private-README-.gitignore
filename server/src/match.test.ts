...
import { Card, RANKS, deal } from '../../app/src/shared/cards';
...
// ── Тараалтын шударга байдал ───────────────────────────────────────────────

test('тараалт суудал хооронд хазайхгүй', () => {
  // Тоглогч "нэг хүнд дандаа сайн хөзөр ирээд байна" гэж мэдэгдсэн тул
  // холилтыг тоогоор бэхэлж байна. Fisher-Yates эвдэрвэл энэ тест унана.
  const rng = mulberry32(2026);
  const DEALS = 20_000;
  const SEATS = 4;

  const strength = Array<number>(SEATS).fill(0);
  const topCard = Array<number>(SEATS).fill(0);

  for (let d = 0; d < DEALS; d += 1) {
    const hands = deal(SEATS, rng);
    for (let s = 0; s < SEATS; s += 1) {
      for (const card of hands[s]) {
        strength[s] += card;
        if (card === 51) topCard[s] += 1; // 2♠ — хамгийн хүчтэй хөзөр
      }
    }
  }

  // Гар бүрийн индексийн нийлбэрийн онолын дундаж: 13 × 25.5 = 331.5
  const averages = strength.map((sum) => sum / DEALS);
  const spread = Math.max(...averages) - Math.min(...averages);
  assert.ok(spread < 3, `суудал хоорондын хүчний зөрүү хэт их: ${spread.toFixed(2)}`);
  for (const avg of averages) {
    assert.ok(Math.abs(avg - 331.5) < 3, `дундаж хүч онолоос хазайв: ${avg.toFixed(2)}`);
  }

  // 2♠ суудал бүрд ижил магадлалтай ирэх ёстой (χ², 3 чөлөөний зэрэг).
  const expected = DEALS / SEATS;
  const chi = topCard.reduce((sum, n) => sum + (n - expected) ** 2 / expected, 0);
  assert.ok(chi < 11.34, `2♠-ийн тархалт хазайв: χ² = ${chi.toFixed(2)}`);
});

test('тараасан хөзөр давхардахгүй, бүрэн байна', () => {
  const rng = mulberry32(7);
  for (let d = 0; d < 500; d += 1) {
    for (const seats of [2, 3, 4]) {
      const hands = deal(seats, rng);
      assert.equal(hands.length, seats);
      const all = hands.flat();
      assert.equal(all.length, seats * 13, 'хөзрийн тоо буруу');
      assert.equal(new Set(all).size, all.length, 'давхардсан хөзөр гарлаа');
      for (const card of all) {
        assert.ok(Number.isInteger(card) && card >= 0 && card <= 51, `хүчингүй хөзөр: ${card}`);
      }
      for (const hand of hands) {
        const sorted = [...hand].sort((a, b) => a - b);
        assert.deepEqual(hand, sorted, 'гар эрэмбэлэгдээгүй байна');
      }
    }
  }
});
